...
### [WIZ-STEPS-EDITOR-01] Editor visual para `wizard_config.steps` en Estructura de colección (2026-07-22)

David preguntó si valía la pena construir una UI para editar `wizard_config.steps` (hasta ahora solo fijable cargando uno de los 5 presets de `CmsPresetCatalog.php`, sin forma de personalizarlo por colección). Investigué antes de proponer un plan (ver plan aprobado en la sesión): `wizard_config.steps[].fields[].key` fuera de los 5 native keys reconocidos termina en `wizard_extra`, un JSON transitorio que el Domain solo usa para auto-rellenar un campo de bloque si el nombre coincide exactamente — sin eso, la clave queda inerte para siempre. Conclusión: el editor NO debe permitir claves libres, solo debe ordenar/agrupar un catálogo fijo de 6 atributos reales de la entrada (`title`, `excerpt`, `featured_image`, `meta_title`, `meta_description`, `og_image`) que sí tienen dónde persistir.

**Domain:** nuevas `App\Libraries\Cms\WizardStepFieldCatalog` (catálogo fijo key→type) y `App\Validators\WizardConfigValidator` (mirror exacto de `BlockTemplateValidator`) — valida forma de `steps`, que cada `key` esté en el catálogo, que el `type` coincida, que no se repita una key en más de un paso, y que `title` esté presente en algún paso. Cableado en `CollectionCreateRequestDTO`/`CollectionUpdateRequestDTO` igual que `BlockTemplateValidator` ya estaba cableado — antes `wizard_config` no tenía NINGUNA validación de forma server-side. Tests: `WizardConfigValidatorTest` (11 casos) + 2 casos nuevos en `CollectionCreateRequestDTOValidationTest`.

**Admin:** `entryPublish.js` gana soporte `og_image` (mismo patrón dual que `featured_image`, antes inexistente en el Wizard) y suma `meta_title`/`meta_description`/`og_image` a `NATIVE_KEYS` (ya se enviaban correctamente a `translations`, pero se duplicaban también en `wizard_extra` por no estar excluidos). Nueva pestaña "Pasos del asistente" en `block_template_editor.php` (junto a Catálogo/Estructura, mismo componente Alpine `collectionBlockTemplateBuilder`): agregar/quitar pasos, agregar/quitar campos del catálogo fijo dentro de cada paso, reordenar pasos, editar título/ayuda de paso y `required` por campo. `sync()` dejó de hacer passthrough verbatim de `wizard_config` — ahora lo reconstruye desde el nuevo estado `stepRows` (mirror de `buildTemplate()` para bloques). Banner de aviso (no bloqueante, igual que el `valid` ya existente de bloques nunca bloqueó el submit) cuando `title` queda fuera de todos los pasos — el backstop real es el 422 del `WizardConfigValidator`. Nuevos tests `testUpdateStructurePersistsWizardConfigStepsRoundTrip`/`testUpdateStructureSurfacesWizardConfigValidationErrorFromApi` en `CollectionFlowTest.php`.

**Bug propio encontrado y corregido durante la verificación en navegador:** al probar interactivamente vía consola, un primer intento usó `querySelectorAll('select')` sin acotar al panel nuevo — encontró el `<select>` de `block_key` del bloque bloqueado ("Titular"/`rich_text`) en la pestaña Estructura, le disparé un `change` con un valor inexistente, y `row.block_key` quedó en `''`, lo que `buildTemplate()` filtra silenciosamente al construir el JSON de salida. Al enviar el formulario esto persistió un `block_template` con 1 solo bloque en la colección real de Noticias. Detectado inmediatamente comparando el conteo de bloques antes/después; corregido restaurando vía `php spark db:seed NewsCollectionSeeder` y repitiendo la verificación acotando los selectores exclusivamente al nuevo panel (`x-ref="wizardStepsList"`) y confirmando `rows`/bloques sin cambios antes de cada submit real. No es un bug del código enviado — fue un error de mi propio método de prueba vía consola; documentado aquí porque tocó datos reales de dev.

**Verificación:** admin `composer test` (621/621, 1 skip conocido), `composer analyse` (PHPStan 204 archivos, 0 errores), `composer format:check` + `i18n-check` limpios, `npm run test:js` (83/83), `npm run lint:js` limpio, `npm run build:all` sin errores. Domain `composer test` (suite completa Unit/Architecture/Integration/Feature/SeederContracts, exit 0), `composer analyse`/`format:check` limpios. Verificado en navegador real sobre Noticias: combinar/quitar campos vía los métodos reales de Alpine (no manipulación cruda del DOM) preservando los 2 bloques intactos; guardado real end-to-end (`meta_title` agregado → verificado en BD → revertido → verificado en BD); rechazo real end-to-end de un `wizard_config` con clave inventada (`custom_field`) — confirmado que el Domain lo rechaza (mensaje "not part of the native field catalog" presente en la respuesta) y que NO se persiste.
