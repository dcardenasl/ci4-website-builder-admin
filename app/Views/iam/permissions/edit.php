...
<div class="mb-4 flex items-center justify-between">
    ...
    <form method="post" action="<?= route_to('admin.iam.permissions.delete', (string) ($item['id'] ?? '')) ?>" onsubmit="return confirm('<?= esc(confirm_delete_message($item['code'] ?? $item['resource'] ?? null), 'js') ?>');">
        <?= csrf_field() ?>
