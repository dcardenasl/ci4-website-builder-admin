...
$labelsJson = json_encode([
    ...
    'filter_operator_equals' => lang('Pages.listing_projection_filter_operator_equals'),
    'filter_operator_contains' => lang('Pages.listing_projection_filter_operator_contains'),
    'filter_operator_before' => lang('Pages.listing_projection_filter_operator_before'),
    'filter_operator_after' => lang('Pages.listing_projection_filter_operator_after'),
    'filter_operator_in' => lang('Pages.listing_projection_filter_operator_in'),
...
<div class="border-t border-gray-100 pt-5"
    ...
    <div class="rounded-2xl border border-slate-200 bg-slate-50/70 p-5 space-y-5">
        ...
        <div class="border-t border-slate-200 pt-4"><div class="flex items-center justify-between gap-3"><div><h5 class="text-xs font-semibold text-slate-800"><?= esc(lang('Pages.listing_projection_filters_title')) ?></h5><p class="mt-1 text-[11px] text-slate-500"><?= esc(lang('Pages.listing_projection_filters_description')) ?></p></div><button type="button" @click="addFilter()" :disabled="projection.filters.length >= 6" class="inline-flex items-center gap-1.5 rounded-lg border border-brand-200 bg-white px-3 py-2 text-xs font-semibold text-brand-700 disabled:cursor-not-allowed disabled:opacity-50"><span aria-hidden="true">+</span> <?= esc(lang('Pages.listing_projection_add_filter')) ?></button></div><div class="mt-3 space-y-2"><template x-for="(item, index) in projection.filters" :key="item.id"><div class="grid gap-2 rounded-xl border border-slate-200 bg-white p-3 md:grid-cols-[minmax(0,1fr)_10rem_minmax(0,1fr)_auto]"><select class="min-w-0 rounded-lg border border-slate-300 px-3 py-2 text-sm" :value="item.source" @change="setFilterSource(item, $event.target.value)"><option value=""><?= esc(lang('Pages.listing_projection_option_select')) ?></option><template x-for="field in availableFields({ filterable: true, publicFilter: true })" :key="field.value"><option :value="field.value" x-text="field.label"></option></template></select><select class="rounded-lg border border-slate-300 px-3 py-2 text-sm" :value="item.operator" @change="item.operator = $event.target.value"><template x-for="operator in operatorOptions(item.source)" :key="operator.value"><option :value="operator.value" x-text="operator.label"></option></template></select><input type="text" class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm" placeholder="<?= esc(lang('Pages.listing_projection_filter_label_placeholder'), 'attr') ?>" x-model="item.label"><button type="button" @click="removeFilter(index)" class="rounded-lg px-2 py-2 text-xs font-medium text-red-600"><?= esc(lang('Pages.listing_projection_remove')) ?></button></div></template><p x-show="projection.filters.length === 0" class="rounded-xl border border-dashed border-slate-300 px-4 py-3 text-xs text-slate-500"><?= esc(lang('Pages.listing_projection_filters_empty')) ?></p></div></div>
