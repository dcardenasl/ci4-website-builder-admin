...
<div class="mb-4 flex items-center justify-between">
    ...
    <form method="post" action="<?= route_to('admin.cms.redirects.delete', (string) ($item['id'] ?? '')) ?>" x-data @submit.prevent="$store.confirm.show('<?= esc(confirm_delete_message($item['old_path'] ?? $item['new_url'] ?? null), 'js') ?>', () => $el.submit())">
        <?= csrf_field() ?>
...
<?php ob_start(); ?>
<form method="post" action="<?= route_to('admin.cms.redirects.update', (string) ($item['id'] ?? '')) ?>" class="space-y-6">
        <?= csrf_field() ?>
