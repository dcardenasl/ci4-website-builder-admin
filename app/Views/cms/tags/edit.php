...
<div class="mb-4 flex items-center justify-between">
    ...
    <form method="post" action="<?= route_to('admin.cms.tags.delete', (string) ($item['id'] ?? '')) ?>" x-data @submit.prevent="$store.confirm.show('<?= esc(confirm_delete_message($item['name'] ?? $item['slug'] ?? null), 'js') ?>', () => $el.submit())">
        <?= csrf_field() ?>
...
<?php ob_start(); ?>
<form method="post" action="<?= route_to('admin.cms.tags.update', (string) ($item['id'] ?? '')) ?>" class="space-y-6">
        <?= csrf_field() ?>
