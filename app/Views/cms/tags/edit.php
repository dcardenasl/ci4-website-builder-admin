...
<div class="mb-4 flex items-center justify-between">
    ...
    <form method="post" action="<?= route_to('admin.cms.tags.delete', (string) ($item['id'] ?? '')) ?>" onsubmit="return confirm('<?= esc(confirm_delete_message($item['name'] ?? $item['slug'] ?? null), 'js') ?>');">
        <?= csrf_field() ?>
...
<section class="bg-white border border-gray-200 rounded-xl shadow-sm p-5 max-w-3xl">
    <h3 class="text-lg font-semibold text-gray-900"><?= esc(lang('Tags.tags_edit')) ?></h3>

    <form method="post" action="<?= route_to('admin.cms.tags.update', (string) ($item['id'] ?? '')) ?>" class="mt-4 space-y-4">
        <?= csrf_field() ?>
