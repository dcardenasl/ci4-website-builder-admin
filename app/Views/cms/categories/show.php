...
<div class="mb-4">
    <a href="<?= route_to('admin.cms.categories') ?>" class="text-sm text-brand-600 hover:text-brand-700">&larr; <?= lang('Categories.categories_title') ?></a>
</div>
...
<?php elseif (! empty($category)): ?>
    ...
    <section class="bg-white border border-gray-200 rounded-xl shadow-sm p-5 max-w-3xl">
        <div class="flex items-center justify-between">
            <h3 class="text-lg font-semibold text-gray-900"><?= lang('Categories.categories_details') ?></h3>
            <div class="flex items-center gap-2">
                ...
                <form method="post" action="<?= route_to('admin.cms.categories.delete', $itemId) ?>" onsubmit="return confirm('<?= esc(confirm_delete_message($category['name'] ?? $category['slug'] ?? null), 'js') ?>');">
                    <?= csrf_field() ?>
        ...
        <?php if (! empty($category['translations']) && is_array($category['translations'])): ?>
            <div class="mt-6 border-t border-gray-200 pt-6">
                <h4 class="text-md font-semibold text-gray-800"><?= esc(lang('Categories.translations_title')) ?></h4>
